...
# ============================================================
# Ejercicio 1 (necesario: el Ejercicio 4 compara elitismo simple vs ampliado
# usando cubic_ga, decode_cubic y fitness_cubic definidos aquí)
# ============================================================
...
# Ejercicio 4
# ============================================================
"""## Ejercicio 4: Elitismo Ampliado (3 mejores individuos)

Se crea una subclase `AlgoritmoGeneticoElitismoAmpliado` que hereda de
`AlgoritmoGenetico` y modifica la función interna que maneja el elitismo:
en lugar de preservar solo al mejor individuo de la generación actual
(elitismo simple, como en el Módulo 1), se preservan de manera INTACTA los
`elite_size` (3 por defecto) mejores individuos de la generación actual
hacia la siguiente generación, reemplazando a los 3 individuos más débiles
de la población recién generada.

Se usa una subclase (en vez de modificar directamente `AlgoritmoGenetico`)
para no alterar el comportamiento de elitismo simple usado en los
Ejercicios 1, 2 y 3.

NOTA: Esta celda depende del Ejercicio 1 — reutiliza `cubic_ga`, `decode_cubic`,
`fitness_cubic`, `POP_SIZE_CUBIC`, `PC_CUBIC`, `PM_CUBIC` y
`NUM_GENERATIONS_CUBIC` para poder comparar el elitismo simple contra el
elitismo ampliado sobre el mismo problema. Ejecútala después del Ejercicio 1.
"""

class AlgoritmoGeneticoElitismoAmpliado(AlgoritmoGenetico):
    """
    Variante de AlgoritmoGenetico donde, en lugar de preservar solo al mejor
    individuo (elitismo simple), se preservan de forma intacta los
    `elite_size` mejores individuos de la generación actual hacia la
    siguiente, reemplazando a los `elite_size` individuos más débiles de
    la nueva población generada por selección, cruzamiento y mutación.
    """

    def __init__(self, *args, elite_size: int = 3, **kwargs):
        super().__init__(*args, **kwargs)
        self.elite_size = elite_size  # Número de individuos élite a preservar cada generación.

    def _apply_elitism_ampliado(
        self,
        ...
        """
        Reemplaza a los `elite_size` individuos más débiles de la nueva
        población por los `elite_size` mejores individuos (copiados de
        forma intacta) de la generación anterior.

        Principios Bioinspirados en el código:
        *   **Elitismo Ampliado**: se garantiza que varios de los mejores
            individuos (no solo uno) sobrevivan sin alteración a la
            siguiente generación, reduciendo el riesgo de perder buen
            material genético por efecto del cruzamiento o la mutación.
        """
        # Índices de los individuos de la generación anterior, ordenados de mayor a menor aptitud.
        sorted_old_indices = np.argsort(old_fitness)[::-1]
        elite_indices = sorted_old_indices[: self.elite_size]
        elites = [copy.deepcopy(old_population[i]) for i in elite_indices]

        # Se evalúa la nueva población para identificar a los individuos más débiles a reemplazar.
        new_fitness, _ = self._calculate_all_fitness(new_population)
        sorted_new_indices = np.argsort(new_fitness)  # De menor a mayor aptitud.
        worst_indices = sorted_new_indices[: self.elite_size]
    ...
    def run(self, num_generations: int) -> tuple[list[int], float]:
        """
        Ejecuta el AG igual que la clase base (AlgoritmoGenetico.run), pero
        aplicando elitismo ampliado en el paso 4: se preservan los
        `elite_size` mejores individuos de la generación actual, no solo 1.
        """
        self._initialize_population()

        for generation in range(num_generations):
            # 1. Evaluar la aptitud de la población actual (generación t).
            fitness_values, _ = self._calculate_all_fitness(self.population)
            ...
            # Se guarda la población y el fitness de la generación actual ANTES de
            # reemplazarla, ya que se necesitan para extraer a los `elite_size` mejores.
            old_population = copy.deepcopy(self.population)
            old_fitness = fitness_values

            # 2. Selección (idéntica a la clase base).
            if self.selection_method == 'roulette':
                mating_pool = self._select_proportional(self.population, fitness_values)
            else:
                mating_pool = self._select_tournament(self.population, fitness_values)

            # 3. Cruzamiento y Mutación (idénticos a la clase base).
            new_population = []
            if len(mating_pool) % 2 != 0:
                mating_pool.append(random.choice(mating_pool))
            ...
            # 4. Elitismo Ampliado: preservar los `elite_size` mejores de la generación anterior.
            if self.elitism:
                new_population = self._apply_elitism_ampliado(old_population, old_fitness, new_population)

            self.population = new_population

        return self.best_individual_genotype, self.best_individual_fitness

# --- Ejecución del AG con Elitismo Ampliado sobre el problema del Ejercicio 1 ---

print("\n--- Ejecutando AG con Elitismo Ampliado (3 mejores) sobre el Ejercicio 1 ---")
elitism_ga = AlgoritmoGeneticoElitismoAmpliado(
    population_size=POP_SIZE_CUBIC,
    chromosome_length=CHROM_LEN_CUBIC,
    pc=PC_CUBIC,
    pm=PM_CUBIC,
...
best_genotype_elite, best_fitness_elite = elitism_ga.run(NUM_GENERATIONS_CUBIC)
best_x_elite = decode_cubic(best_genotype_elite)

print(f"Mejor x encontrado: {best_x_elite:.4f} | Mejor Fitness: {best_fitness_elite:.4f}")
print(f"Óptimo teórico esperado: x = 1.0, f(1) = 2.0")

# --- Comparación: Elitismo Simple (Ejercicio 1) vs Elitismo Ampliado ---
plt.figure(figsize=(7, 5))
plt.plot(
    range(1, NUM_GENERATIONS_CUBIC + 1),
    cubic_ga.avg_fitness_history,
    label='Fitness Promedio - Elitismo Simple (1 individuo)',
)
plt.plot(
    range(1, NUM_GENERATIONS_CUBIC + 1),
    elitism_ga.avg_fitness_history,
    label='Fitness Promedio - Elitismo Ampliado (3 individuos)',
)
plt.title('Comparación: Elitismo Simple vs Elitismo Ampliado')
plt.xlabel('Generación')
